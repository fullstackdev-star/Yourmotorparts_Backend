...
var MessagingView = Backbone.View.extend({
    ...
    onLoad: function(){
        ...
        // Room name
        $("#room_name").html(LoginUserManager.room_name);
        ...
        Backbone.on(CONST.EVENT_ON_MESSAGE_UPDATED,function(obj){
            _.forEach(obj,function(updatedMessage){
                ...
                messageModel.set('status', CONST.MESSAGE_STATUS_SENT);
        ...
        Backbone.on(CONST.EVENT_ON_TYPING, function(obj){
            if(obj.user_id == LoginUserManager.user.get('id'))
                return;
        ...
        SS( "#text-message-box" ).on('change keyup paste',function(){
            ...
            if(self.lastTextLength == 0 && length > 0) {
                socketIOManager.emit(CONST.EMIT_KEYWORD_SEND_TYPING, {
        ...
        SS('#btn-fileupload').on('click',function(){
            console.log("btn-fileupload");
            self.fileUploader.handleClick();
        });
        
        SS('#file-input').on('change',function(event){
            console.log("file-input");
            self.fileUploader.startUploadingFile(event);
        });
    ...
    loadNextMessage: function(){
        ...
        this.isLoading = true;
        WebAPIManager.get(
            UrlGenerator.messageList(LoginUserManager.room_id, lastMessageId),
            // success
            function(data) {
    ...
    newMessage:function(obj) {
        ...
        console.log('MessagingView.js:338', newMessage);

        newMessage.set('status', CONST.MESSAGE_STATUS_SENT);
                
        var tempMessageExists = false;
        if(newMessage.get('user_id') == LoginUserManager.user.get('id')){
            // swap message
            this.messages.each(function(oldMessage, index) {
                if(!_.isEmpty(oldMessage.get('local_id')) &&
                    !_.isEmpty(newMessage.get('local_id')) &&
                    oldMessage.get('local_id') == newMessage.get('local_id')){
    ...
    addTyping: function(obj){
        var emlContainer = SS('#additional-notification-container');
        var userIDEscapted = encodeURIComponent(obj.user_id).replace("'","quote").replace("%","");

        var text = obj.user.username + " is typing...";
    ...
    sendOpenMessage:function(messages){
        ...
        _.forEach(messages,function(message){
            if(message.get('user_id') != LoginUserManager.user.get('id')){
    ...
    updateMessage:function(newMessage) {
        console.log('MessagingView.js:611', newMessage);
        ...
        if(_.isEmpty(oldMessage)){
            oldMessage = this.messages.findMessageByLocalID(newMessage.get('local_id'));
        }
    ...
    deleteMessage:function(messageId) {
        ...
        // get old message
        var oldMessage = this.messages.findMessageByID(messageId);

        if(_.isEmpty(oldMessage)){
            return;
        }

        this.messages.delete(oldMessage);

        SS('#' + oldMessage.get('id')).remove();
    }
