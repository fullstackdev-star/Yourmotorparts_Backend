...
class image_CRUD {
	...
	function set_author($author_field, $author_id) {
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
        ...
        if( !empty($this->author_id) ) {
            $insert[$this->author_field] = $this->author_id;
        }

        $this->ci->db->insert($this->table_name, $insert);
        $insert_id = $this->ci->db->insert_id();
        if( $this->callback_after_insert ) {
            $callback_return = call_user_func($this->callback_after_insert, $insert, $insert_id);
        }
    }
    ...
    protected function _delete_file($id)
    {
    	...
    	$result = $this->ci->db->delete($this->table_name, array($this->primary_key => $id));
        if($result && $this->callback_after_delete) {
            $callback_return = call_user_func($this->callback_after_delete, $id);
        }
    }
