		...
		if ($form->validate())
		{
			...
			if ($user)
			{
				if (!$this->config->item('use_ci_email', 'ion_auth'))
				{
					...
					//$this->my_email->my_send($identity, $subject, $email_view, $user);
                    $this->my_email->send_email_template($identity, APP_NAME, $subject, $email_view, $user);
				}
			...
			if ($form->validate())
			{
				...
				if ( $this->ion_auth->reset_password($identity, $password) )
				{
					if (!$this->config->item('use_ci_email', 'ion_auth'))
					{
						...
						$this->my_email->send($identity, $subject, $email_view, $data);
					}
