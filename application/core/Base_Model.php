...
class Base_Model extends CI_Model
{
    ...
    private function _fetch_table()
    {
        if ($this->_table == NULL)
        {
            ...
            if( $this->endsWith( $class_name, 'key_m' ) || $this->endsWith( $class_name, 'key_model' ) ){
                $this->_table = preg_replace('/(_m|_model)?$/', '', $class_name).'s';
            } else if( $this->endsWith( $class_name, 'y_m' ) || $this->endsWith( $class_name, 'y_model' ) ){
                $this->_table = preg_replace('/(y_m|y_model)?$/', '', $class_name).'ies';
            } else if( $this->endsWith( $class_name, 'ss_m' ) || $this->endsWith( $class_name, 'ss_model' ) ){
                $this->_table = preg_replace('/(_m|_model)?$/', '', $class_name).'es';
            } else if( $this->endsWith( $class_name, 's_m' ) || $this->endsWith( $class_name, 's_model' ) ){
                $this->_table = preg_replace('/(_m|_model)?$/', '', $class_name);
            } else{
                $this->_table = preg_replace('/(_m|_model)?$/', '', $class_name).'s';
            }
        }
    }
    function endsWith($string,$endString,$case=true)
    {
        $expectedPosition = strlen($string) - strlen($endString);

        if ($case)
            return strrpos($string, $endString, 0) === $expectedPosition;

        return strripos($string, $endString, 0) === $expectedPosition;
    }
