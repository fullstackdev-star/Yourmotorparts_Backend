...
class Admin_Controller extends MY_Controller {
	...
	protected function render($view_file, $layout = 'default')
	{
        ...
        $this->load->model('Report_message_model', 'messages');
        $new_messages = $this->messages->get_where('status', '0');
        $this->mViewData['new_messages'] = $new_messages;
	...
	protected function generate_image_crud($table, $url_field, $upload_path, $category_field = '', $order_field = 'pos', $title_field = '')
	{
		// create CRUD object
        ...
        {
            $crud->set_relation_field($category_field);
        }

        /*if( $this->mUser->group_id > 0) {
            $crud->set_author('team_id', $this->mUser->group_id);
        }*/
