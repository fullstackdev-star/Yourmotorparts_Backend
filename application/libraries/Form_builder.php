...
class Form {
    ...
    public function field_radio($name, $value = NULL, $checked = FALSE, $extra = array())
    {
        $data = array('type' => 'radio', 'name' => $name);
        $value = ($value===NULL) ? $this->get_field_value($name) : $value;
    ...
    public function bs3_radio($label, $name, $values = array(), $extra = array())
    {
        $extra['class'] = 'form-control';
        $result = '<div class="form-group">'.form_label($label, $name).'</br>';
        $i=0;
        foreach ($values as $value) {
            $result .= '<tr><td>'.$this->field_radio($name, $value["value"], FALSE/*$i==0?TRUE:FALSE*/).'</td><td>'.form_label($value["label"], $name).'</td></tr>'.'&nbsp;&nbsp;&nbsp;';
            $i++;
        }
        return $result.'</p>';
    }

    public function bs3_upload($label, $name = 'upload_image', $value = NULL, $extra = array())
    {
        return '<div class="form-group">'.form_label($label, $name).$this->field_upload($name, $value, $extra).'</div>';
    }
